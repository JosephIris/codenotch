...
For the matching Stream Deck **Codex Usage** key, quit Stream Deck and run `./Windows/streamdeck/install-codex.ps1`, then reopen Stream Deck and drag **Claude Usage → Codex Usage** beside the Claude keys. The mint gauge displays percentage used, window length, and reset countdown. Press it to cycle windows when the account reports more than one. Keep Codenotch running: the key reads its existing cache every 15 seconds and makes no extra OpenAI requests. Old readings are marked **STALE**; missing usage shows **Open Codenotch**, never a made-up zero.
